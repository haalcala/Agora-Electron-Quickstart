		...
		return (
			<div className="card" style={{ border: "1px solid red", width: "100%", height: "100%", padding: "1em", height: "-webkit-fill-available"}}>
				{/* <div className="card" style={{padding: "10px"}}> */}
					...
					<div className="card" style={{border: "1px solid green", height: "23em", display: "block"}}>
						<div style={{margin: "1em"}}>
							{_.times(4).map(i => {
								return (
									<button key={i} className={"answer-item is-link" + (selected_answer === i ? " selected": "")} onClick={() => this.selectAnswer(i)}>{options[i]}</button>
								);
							})}
						</div>
					</div>
